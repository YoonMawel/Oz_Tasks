...
객체를 생성해야 할 때 클래스가 직접 객체를 생성하는 것이 아니라, 객체 생성을 위한 팩토리 메서드를 제공하여 서브 클래스에서 해당 메서드를 오버라이드(위임)하고 적절한 객체를 생성할 수 있도록 한다.

1번 서비스인가, 2번 서비스인가? 처럼 어떤 객체를 생성해서 사용할지를 외부에서 결정할 때 팩토리 메서드를 사용함 
